...
async function exportMemories(scope) {
  ...
  if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
    try {
      ...
      return pts.slice(0, outLimit).map(p => { const doc = payloadToRow(p.id, p.score, p.payload); delete doc.history; delete doc.entities; delete doc.relations; delete doc.entity_names; return doc; });
    } catch (e) { return []; }
  }
  const d = sqliteInit();
  ...
  return rows.map(r => { const doc = rowToDoc(r); delete doc.history; delete doc.entities; delete doc.relations; delete doc.entity_names; return doc; });
}
