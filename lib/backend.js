...
// 全局错误计数（静默 catch 处也有了观测能力）
const errStats = { embed: 0, fts: 0, kg: 0, webhook: 0, bump: 0, changelog: 0, cleanup: 0, other: 0 };
...
function ftsUpsert(id, content, tags, project, user, session) {
  ...
  } catch (e) { errStats.fts++; }
}
function ftsDelete(id) {
  try { sqliteInit().prepare('DELETE FROM memory_fts WHERE id=?').run(id); } catch (e) { errStats.fts++; }
}
...
module.exports = {
  ...
  dedupStats, errStats, backfillContentHash,
