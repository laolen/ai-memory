...
async function exportMemories(scope) {
  ...
  if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
    try {
      ...
      return pts.map(p => { const doc = payloadToRow(p.id, p.score, p.payload); delete doc.history; delete doc.entities; delete doc.relations; delete doc.entity_names; return doc; });
    } catch (e) { return []; }
  }
  const d = sqliteInit();
  ...
  if (scope.project) { where.push('project=?'); params.push(scope.project); }
  if (scope.session) { where.push('session=?'); params.push(scope.session); }
  const limit = Math.min(scope.limit || 100000, 100000);
  const rows = d.prepare('SELECT * FROM memories WHERE ' + where.join(' AND ') + ' ORDER BY created_at LIMIT ?').all(...params, limit);
