...
// v1.22.2: 路径归一化——Qdrant keyword 的 match.value 无法匹配含反斜杠的字符串，
// 故所有 project 在「存储」与「查询」两侧都统一把反斜杠归一为正斜杠
// （Windows 工作区路径 D:\x\y → D:/x/y），保证索引命中与跨端一致性。
// null/undefined/'' 原样返回（'' 在调用处会被 falsy 跳过，等价「无项目」桶）。
function normalizeProject(p) {
  if (typeof p !== 'string') return p;
  return p.replace(/\\/g, '/');
}

// 全局错误计数引用（定义在 config.js，全模块共享）
...
function sqliteInit() {
  ...
  // v1.12.0: 多主体归属维度（群聊/多 Agent 场景记录「谁说的/哪个 Agent/哪次运行」）
  if (!_cols.has('actor_id')) db.exec('ALTER TABLE memories ADD COLUMN actor_id TEXT');
  if (!_cols.has('agent_id')) db.exec('ALTER TABLE memories ADD COLUMN agent_id TEXT');
  if (!_cols.has('run_id')) db.exec('ALTER TABLE memories ADD COLUMN run_id TEXT');
  // v1.13.0: 记忆固定(pin) 标记——免除过期/衰减，永远保留
  if (!_cols.has('pinned')) db.exec("ALTER TABLE memories ADD COLUMN pinned INTEGER DEFAULT 0");
  // v1.13.0 修复：content_hash——归一化 SHA-1 精确判重用
  if (!_cols.has('content_hash')) db.exec("ALTER TABLE memories ADD COLUMN content_hash TEXT");
  // v1.15.0: 间隔重复（spaced repetition）调度时间戳；Qdrant payload 已有，SQLite 建表/迁移此前漏加，此处补齐
  if (!_cols.has('next_review_at')) db.exec("ALTER TABLE memories ADD COLUMN next_review_at TEXT");
  ...
  // v1.12.0 (gap①): 项目级持久配置——自定义类别体系 + 持久抽取指令 + 默认加权检索标准，一次配置长期生效
  db.exec('CREATE TABLE IF NOT EXISTS project_config (project TEXT PRIMARY KEY, custom_categories TEXT, extract_instructions TEXT, criteria TEXT, webhook_urls TEXT, updated_at TEXT)');
  // v1.21.0: 备份记录表
  db.exec('CREATE TABLE IF NOT EXISTS backups (name TEXT PRIMARY KEY, size INTEGER, created_at TEXT)');
  return db;
}
...
function ftsUpsert(id, content, tags, project, user, session) {
  ...
  } catch (e) { config.errStats.fts++; }
}
function ftsDelete(id) {
  try { sqliteInit().prepare('DELETE FROM memory_fts WHERE id=?').run(id);   } catch (e) { config.errStats.fts++; }
}
...
// v1.13.0 修复：存量记忆 content_hash 回填（挂于 /api/reindex，对全量记忆补算哈希以支持精确判重）
async function backfillContentHash() {
  ...
  if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
    ...
    for (const p of pts) {
      const pl = p.payload || {};
      if (pl.content && !pl.content_hash) {
        batch.push({ id: p.id, content_hash: util.hashContent(pl.content) });
        if (batch.length >= 100) {
          await qdrant.setPayload(batch);
          count += batch.length;
          batch.length = 0;
        }
      }
    }
    if (batch.length) { await qdrant.setPayload(batch); count += batch.length; }
  }
  // SQLite 路径
  const d = sqliteInit();
  const rows = d.prepare('SELECT id, content FROM memories WHERE content IS NOT NULL AND content_hash IS NULL').all();
  for (const r of rows) {
    d.prepare('UPDATE memories SET content_hash=? WHERE id=?').run(util.hashContent(r.content), r.id);
    count++;
  }
  return count;
}
...
function recordChangelog(op, info) {
  if (config.CONFIG.audit_enabled === false) return;
  try {
    const d = sqliteInit();
    d.prepare('INSERT INTO memory_changelog (memory_id, op, ts, user, project, before, after, source_trigger) VALUES (?,?,?,?,?,?,?,?)')
      .run(info.id || null, op, new Date().toISOString(), info.user || null, info.project || null,
        ...
        info.trigger || null);
  } catch (e) { config.errStats.changelog++; }
}
function _clRow(r) {
...
function getChangelogAll(filter) {
  filter = filter || {};
  ...
  try {
    ...
    if (filter.user) { where.push('user=?'); params.push(filter.user); }
    if (filter.project) { where.push('project=?'); params.push(filter.project); }
    if (filter.trigger) { where.push('source_trigger=?'); params.push(filter.trigger); }
    const wsql = where.length ? ' WHERE ' + where.join(' AND ') : '';
    const total = d.prepare('SELECT COUNT(*) AS c FROM memory_changelog' + wsql).get(...params).c;
    const rows = d.prepare('SELECT * FROM memory_changelog' + wsql + ' ORDER BY id DESC LIMIT ? OFFSET ?').all(...params, limit, offset).map(_clRow);
    return { rows, total, limit, offset };
  } catch (e) { return { rows: [], total: 0, limit, offset: 0 }; }
}
function rowToDoc(r) {
...
function sqliteAdd(doc) {
  ...
  d.prepare('INSERT INTO memories (id,content,user,project,session,tags,embedding,created_at,updated_at,history,entities,relations,source,entity_names,type,category,confidence,access_count,last_accessed_at,expires_at,memory_type,correction_count,corrected_at,version,mem_category,tier,org,extract_version,actor_id,agent_id,run_id,pinned,next_review_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
    .run(doc.id, doc.content, doc.user, doc.project, doc.session,
      ...
      doc.actor_id || null, doc.agent_id || null, doc.run_id || null,
      doc.pinned ? 1 : 0, doc.next_review_at || null);
}
...
function sqliteUpdate(id, patch) {
  ...
  if (patch.project !== undefined) { sets.push('project=?'); params.push(normalizeProject(patch.project) || null); }
  ...
  if (patch.next_review_at !== undefined) { sets.push('next_review_at=?'); params.push(patch.next_review_at || null); }
...
function hitsToRows(res) {
  return res.hits.hits.map(h => ({
    ...
    actor_id: h._source.actor_id || null,
    agent_id: h._source.agent_id || null,
    run_id: h._source.run_id || null }));
}
...
function payloadToRow(id, score, payload) {
  ...
  return {
    ...
    pinned: (payload.pinned !== undefined && payload.pinned !== null) ? !!payload.pinned : false,
...
// is_empty:true（布尔，非 {value:true}）表示字段缺失/空；range 对 ISO 字符串按字典序比较，格式统一即可。
function qdrantFilter(p) {
  const must = [];
  if (p.user) must.push({ key: 'user', match: { value: p.user } });
  if (p.project) must.push({ key: 'project', match: { value: normalizeProject(p.project) } });
  ...
  // v1.12.0 (gap②): 多主体归属过滤
  if (p.actor_id) must.push({ key: 'actor_id', match: { value: p.actor_id } });
  if (p.agent_id) must.push({ key: 'agent_id', match: { value: p.agent_id } });
  if (p.run_id) must.push({ key: 'run_id', match: { value: p.run_id } });
...
// 去重命中统计（可观测性）：exact=内容哈希精确命中, vector=向量相似命中, err=查询失败。
// added 计数在各写入入口（doAdd/reconcileFact 决定新建时）自增。
const dedupStats = { exact: 0, vector: 0, err: 0 };

async function dedupFind(vec, scope, opts) {
  scope = scope || {};
  scope.project = normalizeProject(scope.project); // v1.22.2: 查询侧归一化
  if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
    try {
      ...
      if (scope.project !== undefined) {
        ...
        else filter.must.push({ key: 'project', is_empty: true });
      }
      if (scope.session !== undefined) {
        ...
        else filter.must.push({ key: 'session', is_empty: true });
      }
      // v1.13.x: 内容哈希精确去重兜底——完全相同的文本直接命中已有记忆，
      // 不受向量相似度阈值影响（改写/边缘 case 也能正确合并）。
      if (opts && opts.text) {
        const exactFilter = { must: filter.must.concat([{ key: 'content_hash', match: { value: util.hashContent(opts.text) } }]) };
        const ex = await qdrant.query({ vector: vec, filter: exactFilter, limit: 1 });
        if (ex.length && ex[0].payload && ex[0].payload.content) {
          dedupStats.exact++;
          return { id: ex[0].id, similarity: 1, source: ex[0].payload, exact: true };
        }
      }
      ...
      dedupStats.err++;
      // v1.13.x: 以前这里静默吞错返回 null → 查不到 → 新建重复记忆。现改为记 error，
      // 但仍按「未命中」降级，避免把检索故障升级为写入故障。
      console.error('[dedupFind] Qdrant 查询失败（按未命中处理，可能产生重复记忆）:', e && e.message);
    }
    return null;
  }
...
// v1.13.0 优化：60 秒内对同一 id 只 bump 一次（_bumpCache），避免同一检索周期内重复 HTTP
const _bumpCache = new Map();
setInterval(() => { _bumpCache.clear(); }, 60000).unref();
async function bumpAccess(ids) {
  if (!ids || !ids.length) return;
  const now = new Date().toISOString();
  const deduped = ids.filter(id => { if (_bumpCache.has(id)) return false; _bumpCache.set(id, now); return true; });
  if (!deduped.length) return;
  try {
    if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
      const BATCH = 5; // 分批并发，避免 Qdrant 瞬间负载
      for (let i = 0; i < deduped.length; i += BATCH) {
        await Promise.all(deduped.slice(i, i + BATCH).map(id => qdrant.incrAccess(id, now).catch(() => {})));
      }
      return;
    }
    const d = sqliteInit();
    const ph = deduped.map(() => '?').join(',');
    // v1.15.0: SQLite 路径也更新 next_review_at（间隔重复）
    d.prepare('UPDATE memories SET access_count = access_count + 1, last_accessed_at = ?, next_review_at = ? WHERE id IN (' + ph + ')').run(now, new Date(Date.now() + 720 * 3600000).toISOString(), ...deduped);
  } catch (e) { config.errStats.bump++; }
}

// v1.10.0: 实体词汇表（canonical 名集合），用于查询时实体匹配加权。
// 启动时不主动全量 scrollAll（仅由首次写入时 addEntityVocab 逐步构建），
// 第一次 queryEntities 时才被动做一次全量刷新。
let _entityVocab = null;
let _vocabLoading = false;
async function refreshEntityVocab() {
  // 若已有增量数据则跳过全量扫描
  if (_entityVocab && _entityVocab.size > 0) return _entityVocab;
  if (_vocabLoading) return _entityVocab || new Set();
  _vocabLoading = true;
  try {
    const names = new Set();
    if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
...
function getEntityVocab() {
  if (!_entityVocab) refreshEntityVocab().catch(() => {});
  return _entityVocab || new Set();
}
...
function addWorkingMemory(doc) {
  ...
  d.prepare('INSERT OR REPLACE INTO working_memories (id,content,user,project,session,org,tags,created_at,expires_at,memory_type,meta) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
    .run(doc.id, doc.content || '', doc.user || null, normalizeProject(doc.project) || null, doc.session || null, doc.org || null,
...
// v1.12.0 (gap①): 项目级持久配置。custom_categories: string[]（替换默认 mem_category 枚举）；
// extract_instructions: 项目级持久抽取指令（与逐调用指令拼接，逐调用优先）；
// criteria: [{text,weight}]（该项目检索的默认加权标准）；webhook_urls: string[]（项目级事件推送端点）。
function projectConfigGet(project) {
  if (!project) return null;
  project = normalizeProject(project); // v1.22.2: 查询侧归一化
  try {
    const r = sqliteInit().prepare('SELECT * FROM project_config WHERE project=?').get(project);
    if (!r) return null;
    return {
      project: r.project,
      custom_categories: r.custom_categories ? JSON.parse(r.custom_categories) : null,
      extract_instructions: r.extract_instructions || null,
      criteria: r.criteria ? JSON.parse(r.criteria) : null,
      webhook_urls: r.webhook_urls ? JSON.parse(r.webhook_urls) : null,
      updated_at: r.updated_at,
    };
  } catch (e) { return null; }
}
function projectConfigSet(project, cfg) {
  if (!project) throw new Error('project required');
  project = normalizeProject(project); // v1.22.2: 写入侧归一化
  cfg = cfg || {};
  const prev = projectConfigGet(project) || {};
  const merged = {
    custom_categories: (cfg.custom_categories !== undefined) ? cfg.custom_categories : prev.custom_categories,
    extract_instructions: (cfg.extract_instructions !== undefined) ? cfg.extract_instructions : prev.extract_instructions,
    criteria: (cfg.criteria !== undefined) ? cfg.criteria : prev.criteria,
    webhook_urls: (cfg.webhook_urls !== undefined) ? cfg.webhook_urls : prev.webhook_urls,
  };
  sqliteInit().prepare('INSERT INTO project_config (project,custom_categories,extract_instructions,criteria,webhook_urls,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(project) DO UPDATE SET custom_categories=excluded.custom_categories, extract_instructions=excluded.extract_instructions, criteria=excluded.criteria, webhook_urls=excluded.webhook_urls, updated_at=excluded.updated_at')
    .run(project,
      merged.custom_categories ? JSON.stringify(merged.custom_categories) : null,
      merged.extract_instructions || null,
      merged.criteria ? JSON.stringify(merged.criteria) : null,
      merged.webhook_urls ? JSON.stringify(merged.webhook_urls) : null,
      new Date().toISOString());
  return projectConfigGet(project);
}
function projectConfigDelete(project) {
  try { sqliteInit().prepare('DELETE FROM project_config WHERE project=?').run(project); } catch (e) {}
  return { project, deleted: true };
}
function projectConfigList() {
  try { return sqliteInit().prepare('SELECT project FROM project_config ORDER BY project').all().map(r => projectConfigGet(r.project)); }
  catch (e) { return []; }
}

// v1.13.0: 重置所有记忆（清空 memory + FTS + KG + working + KV，但不删 project_config/changelog）
function resetMemories(confirm) {
  if (!confirm) throw new Error('require confirm=true');
  const d = sqliteInit();
  const r = { memories: 0 };
  // 只清除记忆相关表，保留配置和审计
  try { r.memories = d.prepare('DELETE FROM memories').run().changes; } catch (e) {}
  try { d.prepare('DELETE FROM memory_fts').run(); } catch (e) {}
  try { d.prepare('DELETE FROM kg_entities').run(); } catch (e) {}
  try { d.prepare('DELETE FROM kg_relations').run(); } catch (e) {}
  try { d.prepare('DELETE FROM working_memories').run(); } catch (e) {}
  try { d.prepare('DELETE FROM kv_store').run(); } catch (e) {}
  return r;
}

// v1.13.0: 导出记忆为可序列化的行数组（备份/迁移用）。scope={user,project,session,limit}
// 修复：不再剥离 entities/relations/entity_names/history，保证备份可完整恢复图谱与溯源。
async function exportMemories(scope) {
  scope = scope || {};
  scope.project = normalizeProject(scope.project); // v1.22.2: 查询侧归一化
  const outLimit = Math.min(scope.limit || 100000, 100000);
  if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
    try {
      const filter = qdrantFilter(scope);
      const pts = await qdrant.scrollAll(filter);
      return pts.slice(0, outLimit).map(p => payloadToRow(p.id, p.score, p.payload));
    } catch (e) { return []; }
  }
  const d = sqliteInit();
  ...
  if (scope.project) { where.push('project=?'); params.push(scope.project); }
  if (scope.session) { where.push('session=?'); params.push(scope.session); }
  const rows = d.prepare('SELECT * FROM memories WHERE ' + where.join(' AND ') + ' ORDER BY created_at LIMIT ?').all(...params, outLimit);
  return rows.map(r => rowToDoc(r));
}

// v1.13.0: 导入记忆行（恢复备份用）。返回 {imported,errors}
// 修复：按当前主存储路由——Qdrant 模式重新向量化并 upsert（恢复后可在主存储检索到），且写入 KG/FTS 镜像。
async function importMemories(items) {
  if (!Array.isArray(items)) throw new Error('items must be array');
  const useQ = qdrant.useQdrant() && config.CONFIG.embedding_url;
  let imported = 0, errors = 0;
  for (const doc of items) {
    try {
      doc.source = doc.source || { trigger: 'import' };
      if (!doc.id) { doc.id = require('crypto').randomUUID(); }
      doc.created_at = doc.created_at || new Date().toISOString();
      doc.updated_at = doc.updated_at || doc.created_at;
      doc.tags = doc.tags || [];
      if (useQ) { try { doc.embedding = await embed.embed(doc.content); } catch (e) { doc.embedding = undefined; } }
      if (useQ && doc.embedding) {
        doc.project = normalizeProject(doc.project) || null; // v1.22.2: 写入侧归一化
        const payload = Object.assign({}, doc); delete payload.embedding;
        await qdrant.upsert([{ id: doc.id, vector: doc.embedding, payload }]);
      } else {
        sqliteAdd(doc);
      }
      ftsUpsert(doc.id, doc.content, doc.tags, doc.project, doc.user, doc.session);
      kgUpsert(doc.id, doc.entities || [], doc.relations || [], doc.project);
      imported++;
    } catch (e) { errors++; }
  }
...
// v1.21.0: 备份记录管理
function backupRecordAdd(name, size) {
  sqliteInit().prepare('INSERT OR REPLACE INTO backups (name,size,created_at) VALUES (?,?,?)').run(name, size, new Date().toISOString());
}
function backupRecordList() {
  return sqliteInit().prepare('SELECT * FROM backups ORDER BY created_at DESC').all();
}
function backupRecordDelete(name) {
  sqliteInit().prepare('DELETE FROM backups WHERE name=?').run(name);
}

module.exports = {
  sqliteInit, rowToDoc, sqliteAdd, sqliteGet, sqliteDelete, sqliteUpdate, hitsToRows,
  filters, timeFilter, normDate, qdrantFilter, normalizeProject, payloadToRow, pointsToRows,
  dedupFind, bumpAccess, refreshEntityVocab, getEntityVocab, queryEntities, addEntityVocab,
  dedupStats, backfillContentHash, errStats: config.errStats,
  ...
  projectConfigGet, projectConfigSet, projectConfigDelete, projectConfigList,
  resetMemories, exportMemories, importMemories,
  backupRecordAdd, backupRecordList, backupRecordDelete,
};
