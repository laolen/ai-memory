...
async function captureText(text, scope) {
  scope = scope || {};
  scope.project = backend.normalizeProject(scope.project); // v1.22.2: 写入/查询侧归一化
  // v1.22.3: messages 数组入参（结构化对话）→ 拼接为带角色标签的转写文本，便于 LLM 抽取
  if ((!text || !String(text).trim()) && Array.isArray(scope.messages) && scope.messages.length) {
    text = scope.messages
      .map(m => `${(m && m.role ? m.role : 'user').toUpperCase()}: ${m && m.content ? m.content : ''}`)
      .join('\n');
  }
  if (!text || !String(text).trim()) {
    return { captured: 0, skipped: 0, mode: 'empty', items: [], reason: 'no text or messages provided' };
  }
...
let watchOffsets = {};
try { watchOffsets = JSON.parse(fs.readFileSync(path.join(config.ROOT, '.capture.offsets.json'), 'utf8')); } catch (e) { errC.capture++; }
function saveWatchOffsets() { try { fs.writeFileSync(path.join(config.ROOT, '.capture.offsets.json'), JSON.stringify(watchOffsets)); } catch (e) { errC.capture++; } }
async function tailAndCapture(filepath, scope) {
  ...
  } catch (e) { errC.other++; } finally { _capturing.delete(filepath); }
}
function startWatcher() {
  ...
  try {
    ...
    if (st.isFile()) { tailAndCapture(p, scope); _watchers.push(fs.watch(p, () => tailAndCapture(p, scope))); }
    else if (st.isDirectory()) {
      const processDir = () => { try { for (const f of fs.readdirSync(p)) { if (!/\.(log|txt|md|jsonl)$/i.test(f)) continue; tailAndCapture(path.join(p, f), scope); } } catch (e) { errC.capture++; } };
      processDir(); _watchers.push(fs.watch(p, () => processDir()));
    }
  } catch (e) { errC.other++; }
}
function stopWatcher() {
  for (const w of _watchers) try { w.close(); } catch (e) { errC.other++; }
  _watchers = [];
}

module.exports = { captureText, startWatcher, stopWatcher, tailAndCapture, saveWatchOffsets };
