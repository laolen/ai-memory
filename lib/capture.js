...
async function captureText(text, scope) {
  ...
  if (config.CONFIG.reconcile_enabled || config.CONFIG.llm_enabled) {
    let fcts = await facts.extractFacts(text);
  ...
  let candidates = [];
  if (mode === 'llm') {
    try { candidates = (await facts.llmExtract(text)) || []; }
  ...
  for (const c of candidates) {
    ...
    if (scope.source) m.source = util.normalizeSource(scope.source, 'capture');
    try { const r = await memory.doAdd(m); captured++; items.push({ id: r.id, merged: !!r.merged, content: c.content }); }
    catch (e) { skipped++; }
  }
