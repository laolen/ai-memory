...
function loadConfig() {
  ...
  return {
    ...
    llm_proxy_enabled: (f.llm_proxy_enabled !== undefined) ? !!f.llm_proxy_enabled : false,
    // 上游 LLM 地址（留空=复用 llm_url）；支持以 /v1 结尾，自动补 /chat/completions
    llm_proxy_url: f.llm_proxy_url || '',
    // 代理默认模型（客户端请求带 model 时以其为准）
    llm_proxy_model: f.llm_proxy_model || '',
    // 上游 API Key（留空=复用 llm_api_key）
    llm_proxy_api_key: f.llm_proxy_api_key || process.env.LLM_PROXY_API_KEY || '',
    // 响应后是否自动把本轮对话入库（默认开）
    llm_proxy_auto_capture: (f.llm_proxy_auto_capture !== undefined) ? !!f.llm_proxy_auto_capture : true,
    // v1.23.1: 送入抽取的范围。'turn'（默认）=只取本轮 user+assistant，丢弃 system/历史/tool；
    // 'full'=旧行为，整段 user/assistant 历史都送（噪声大、旧轮次会被重复抽取，不推荐）
    llm_proxy_capture_scope: (f.llm_proxy_capture_scope === 'full') ? 'full' : 'turn',
    // 自动捕获归属的项目（缺省时经 X-Project-Path 头或 query.project 覆盖；scoped key 作用域优先）
    llm_proxy_capture_project: f.llm_proxy_capture_project || '',
    // 自动捕获时记忆的归属来源（默认 assistant）
    llm_proxy_user: f.llm_proxy_user || 'assistant',
  };
}
