...
async function recallForContext(a = {}) {
  ...
  let mode = a.mode || (config.CONFIG.embedding_url ? 'semantic' : 'keyword');
...
// #92 导出为人类可读 Markdown：复用 backend.exportMemories，按 group_by 分组。
// group_by ∈ project(默认)/tag/category/date。
async function exportMarkdown(a = {}) {
  ...
  lines.push('# ' + title);
  lines.push('');
  lines.push('> 导出时间：' + new Date().toISOString() + '　|　共 ' + items.length + ' 条　|　分组：' + groupBy);
  lines.push('');
  ...
  return { ok: true, count: items.length, group_by: groupBy, groups: groups.size, markdown };
}
