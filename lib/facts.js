...
// v1.11.0 (gap②+⑧): 支持 extract_version —— v2 在 v1 基础上额外抽取 Mem0 式高层语义类别
// mem_category ∈ {fact, preference, opinion, event, procedure, skill}；并支持 per-call 抽取引导 extract_instructions (gap⑥)。
async function extractFacts(text, opts) {
  if (!config.CONFIG.llm_enabled || !config.CONFIG.llm_url) return null;
  opts = opts || {};
  const ver = opts.extract_version || config.CONFIG.extract_version || 'v2';
  ...
  let sys = 'You are a fact-extraction engine. From the given conversation/notes, extract durable ATOMIC facts worth remembering long-term (one fact per item, no compound sentences). For each fact provide: statement (one self-contained sentence), type (one of: ' + types + '), category (one of: semantic=durable fact/preference, episodic=specific event/decision in a context, procedural=how-to/workflow/communication style), entities (REQUIRED — an array of EVERY named entity / proper noun that appears in this fact: people, organizations/companies, products, technologies/tools, systems, files, places, version numbers and dates. Copy each entity VERBATIM from the original text, preserving its exact wording and language. This array MUST NOT be empty whenever the statement contains any proper noun; only return an empty array for a fact that genuinely mentions no named entity), confidence (0-1), scope (global|project|session), expires_in_days (integer or null; null for durable facts, e.g. 90 for temporary decisions). Ignore chit-chat, greetings, ephemeral content. 重要：保持输入原文的【语言】，不要翻译（中文输入必须输出中文，英文输入输出英文）；实体/专有名词必须与原文逐字一致，不得翻译、缩写或改写。 示例：输入「周明远是青龙数据(QingLong) 的 CTO，2026 年负责把订单系统从 PostgreSQL 迁移到 TiDB」应抽出两条事实，其 entities 分别为 ["周明远","青龙数据","QingLong"] 与 ["周明远","订单系统","PostgreSQL","TiDB"]。 Respond with ONLY JSON: {"facts":[...]}. If nothing worth keeping, return {"facts":[]}. No markdown, no commentary.';
  // v1.12.0 (gap①): 自定义类别体系——opts.custom_categories(string[]) 覆盖默认 6 类枚举（项目级配置注入）
  const customCats = (Array.isArray(opts.custom_categories) && opts.custom_categories.length) ? opts.custom_categories.map(String) : null;
  if (ver === 'v2') {
    const catSpec = customCats
      ? ('mem_category (MUST be exactly one of this custom taxonomy: ' + customCats.join(', ') + ')')
      : 'mem_category (one of: fact=客观事实/数据/配置, preference=用户偏好/喜好/习惯, opinion=观点/主观判断/评价, event=已发生或计划中的具体事件, procedure=操作步骤/SOP/流程, skill=某人会做某事的能力/技能)';
    sys = 'You are a fact-extraction engine (v2). From the given conversation/notes, extract durable ATOMIC facts worth remembering long-term (one fact per item, no compound sentences). For each fact provide: statement (one self-contained sentence), type (one of: ' + types + '), category (one of: semantic=durable fact/preference, episodic=specific event/decision in a context, procedural=how-to/workflow/communication style), ' + catSpec + ', entities (REQUIRED — array of EVERY named entity / proper noun VERBATIM from the original text, preserving exact wording and language; empty array only if no proper noun at all), confidence (0-1), scope (global|project|session), expires_in_days (integer or null). ' + (customCats ? '重点：mem_category 只能从自定义类别中选择，不得发明新类别。' : '重点：务必识别「偏好/观点」类内容并归入 preference/opinion（例如「我喜欢用 Rust」「我觉得 PostgreSQL 比 MySQL 好」）。') + '保持原文语言，实体不得翻译。 Respond with ONLY JSON: {"facts":[...]}. If nothing worth keeping, return {"facts":[]}. No markdown, no commentary.';
  }
  if (opts.extract_instructions) sys += '\n\n额外抽取指令（用户/调用方指定，必须优先遵循）：' + opts.extract_instructions;
  try {
    const content = await embed.chatJSON({ url: config.CONFIG.llm_url, model: config.CONFIG.llm_model, apiKey: config.CONFIG.llm_api_key || null,
    ...
    // v1.12.0: 自定义类别时按自定义枚举校验（大小写不敏感），默认取第一个自定义类别兜底
    const MEM_CAT = customCats ? customCats.map(s => s.toLowerCase()) : ['fact', 'preference', 'opinion', 'event', 'procedure', 'skill'];
    const CAT_DEFAULT = customCats ? MEM_CAT[0] : 'fact';
    return arr.filter(x => x && x.statement).map(x => ({
      ...
      type: (function (t) {
        ...
        return ft.includes(s) ? s : s;
      })(x.type),
      category: ['semantic', 'episodic', 'procedural'].includes(x.category) ? x.category : 'semantic',
      mem_category: MEM_CAT.includes(String(x.mem_category || '').toLowerCase()) ? String(x.mem_category).toLowerCase() : (ver === 'v2' ? CAT_DEFAULT : null),
...
async function reconcileFact(fact, scope) {
  ...
  // v1.11.0 修复：事实抽取管线此前丢弃调用方 tags（只保留 'auto-captured'），
  // 与旧 doAdd 路径行为不一致，且导致按 tags 过滤/检索捕获记忆时落空。此处合并调用方 tags。
  const baseTags = Array.from(new Set(['auto-captured', ...((scope.tags || []).filter(Boolean).map(String))]));
  ...
  const meta = { type: fact.type, category: fact.category || 'semantic', mem_category: fact.mem_category || null,
    confidence: fact.confidence, scope: fact.scope, expires_at, fact_entities: fact.entities || [], memory_type: memType,
    extract_version: scope.extract_version || config.CONFIG.extract_version || 'v1', org: scope.org || null,
    // v1.12.0 (gap②): 多主体归属透传（捕获管线也保留 actor/agent/run）
    actor_id: scope.actor_id || null, agent_id: scope.agent_id || null, run_id: scope.run_id || null };
  ...
  let vec = null;
  try { vec = await embed.embed(fact.statement); } catch (e) { errC.embed++; }
  if (!vec) {
    const r = await memory.doAdd({ content: fact.statement, user: sUser, project: sProject, session: sSession,
      tags: baseTags, source: captureSource, ...meta });
    return { action: 'added', id: r.id };
  }
  const hit = await backend.dedupFind(vec, { user: sUser, project: sProject, session: sSession }, { text: fact.statement });
  ...
  if (rel === 'contradict') {
    ...
    const patch = { content: fact.statement, tags: Array.from(new Set([...(hit.source.tags || []), ...baseTags])),
      updated_at: new Date().toISOString(), type: fact.type, category: fact.category || 'semantic', mem_category: fact.mem_category || 'fact', confidence: fact.confidence, expires_at,
  ...
  const patch = { content: appended, tags: Array.from(new Set([...(hit.source.tags || []), ...baseTags])),
    updated_at: new Date().toISOString(), type: fact.type, category: fact.category || 'semantic', mem_category: fact.mem_category || 'fact', confidence: Math.max(fact.confidence, hit.source.confidence || 0),
...
// 深度记忆巩固：LLM 分析单条记忆，产出推断与疑问
async function inferMemory(id) {
  let cur;
  try { cur = await memory.getMemory(id); } catch (e) { throw Object.assign(new Error('记忆不存在'), { statusCode: 404 }); }
  if (!cur || !cur.content) throw Object.assign(new Error('记忆内容为空'), { statusCode: 400 });
  const url = config.CONFIG.llm_url || config.CONFIG.kg_url;
  if (!url) return { ok: false, message: '未配置 LLM 端点', inferences: [], questions: [] };
  const prompt = '分析以下记忆，输出 JSON（不要围栏）：\n记忆：' + cur.content + '\n标签：' + (cur.tags || []).join(',') + '\n\n{"stable_facts":["确定性事实"],"inferences":["合理推断"],"questions":["待澄清问题"]}';
  try {
    const c = await embed.chatJSON({ url, model: config.CONFIG.llm_model, apiKey: config.CONFIG.llm_api_key || config.CONFIG.kg_api_key || null,
      messages: [{ role: 'system', content: '分析记忆，提取事实/推断/问题。只返回 JSON。' }, { role: 'user', content: prompt }], temperature: 0.3 });
    const result = { stable_facts: Array.isArray(c && c.stable_facts) ? c.stable_facts : [],
      inferences: Array.isArray(c && c.inferences) ? c.inferences : [], questions: Array.isArray(c && c.questions) ? c.questions : [] };
    const now = new Date().toISOString();
    const entry = { ts: now, op: 'infer', before: null, after: result };
    if (cur.history) cur.history.push(entry); else cur.history = [entry];
    try { await memory.doUpdate(id, { history: cur.history }); } catch (e) { errC.other++; }
    return { ok: true, ...result };
  } catch (e) { return { ok: false, message: 'LLM 分析失败: ' + (e.message || e), inferences: [], questions: [] }; }
}

module.exports = { splitSentences, keywordAllowed, llmExtract, shouldCapture, extractFacts, judgeRelation, reconcileFact, inferMemory };
