...
async function extractFacts(text, opts) {
  ...
  if (ver === 'v2') {
    sys = 'You are a fact-extraction engine (v2). From the given conversation/notes, extract durable ATOMIC facts worth remembering long-term (one fact per item, no compound sentences). For each fact provide: statement (one self-contained sentence), type (one of: ' + types + '), category (one of: semantic=durable fact/preference, episodic=specific event/decision in a context, procedural=how-to/workflow/communication style), mem_category (one of: fact=客观事实/数据/配置, preference=用户偏好/喜好/习惯, opinion=观点/主观判断/评价, event=已发生或计划中的具体事件, procedure=操作步骤/SOP/流程, skill=某人会做某事的能力/技能), entities (REQUIRED — array of EVERY named entity / proper noun VERBATIM from the original text, preserving exact wording and language; empty array only if no proper noun at all), confidence (0-1), scope (global|project|session), expires_in_days (integer or null). 重点：务必识别「偏好/观点」类内容并归入 preference/opinion（例如「我喜欢用 Rust」「我觉得 PostgreSQL 比 MySQL 好」）。保持原文语言，实体不得翻译。 Respond with ONLY JSON: {"facts":[...]}. If nothing worth keeping, return {"facts":[]}. No markdown, no commentary.';
  }
  ...
  try {
    ...
    const MEM_CAT = ['fact', 'preference', 'opinion', 'event', 'procedure', 'skill'];
    return arr.filter(x => x && x.statement).map(x => ({
      ...
      mem_category: MEM_CAT.includes(String(x.mem_category || '').toLowerCase()) ? String(x.mem_category).toLowerCase() : (ver === 'v2' ? 'fact' : null),
