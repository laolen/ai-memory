...
// ---- 分析用缓存：智能深化端点（重复/聚类/遗忘曲线）每次都全量 scroll 向量，开销大。
// 加 30s TTL 缓存，避免频繁点击分析时反复拉取。分析场景容忍短暂陈旧（写入后自然过期）。
const ANALYSIS_TTL_MS = 30 * 1000;
let _analysisCache = { ts: 0, data: null };
async function loadAllCached() {
  const now = Date.now();
  if (_analysisCache.data && (now - _analysisCache.ts) < ANALYSIS_TTL_MS) return _analysisCache.data;
  const d = await loadAll();
  _analysisCache = { ts: now, data: d };
  return d;
}
...
async function findDuplicates(threshold, limit) {
  ...
  const mems = await loadAllCached();
...
module.exports = { loadAll, loadAllCached, findDuplicates, clusterTags, forgettingCurve, cosine, jaccard };
