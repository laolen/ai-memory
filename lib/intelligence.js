...
function computeSalience(r) {
  ...
  const acc = Math.min((Number(r.access_count) || 0) / (config.CONFIG.salience_access_k || 10), 1);
  return util.clamp01((config.CONFIG.salience_w_imp || 0.5) * importance + (config.CONFIG.salience_w_acc || 0.5) * acc);
}
function rerankWithContext(rows, query) {
  ...
  rows.forEach(r => {
    ...
    if (config.CONFIG.salience_enabled) {
      const sw = config.CONFIG.salience_score_w || 0.7;
      const blend = sw + (1 - sw) * sal;
