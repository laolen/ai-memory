...
//   - 响应返回/流结束后，把 [ ...incoming.messages, {role:'assistant',content:reply} ]
//       作为 messages 数组异步交给 capture.captureText 入库（不阻塞响应）。
...
// 异步把整段对话交给 capture 入库（不阻塞代理响应）
function fireCapture(messages, assistantContent, project, user) {
  stats.capture_total++;
  if (!config.CONFIG.llm_proxy_auto_capture) return;
  const full = [].concat(messages || []);
  if (assistantContent) full.push({ role: 'assistant', content: assistantContent });
  // 至少要有 user + assistant 两段才有捕获价值
  if (full.length < 2) { stats.capture_total--; return; }
  capture.captureText('', {
    messages: full,
...
module.exports = { handleChatCompletions, handleModels, stats, upstreamUrl };
