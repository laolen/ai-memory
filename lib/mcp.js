...
const TOOLS = [
  ...
  { name: 'purge_memories', zh: '生命周期清理：在可选 user/project/session 范围内删除早于 N 天（按 updated_at）的记忆。expired_only=true 时用配置的 expiry_days，否则用 days 参数。两者皆未设置则不做任何事。', description: 'Lifecycle cleanup: delete memories older than N days (by updated_at) within an optional user/project/session scope. If expired_only is true, uses configured expiry_days; otherwise the days argument is used. Nothing happens if neither is set.',
    inputSchema: { type: 'object', properties: {
      ...
      required: [] } },
  { name: 'capture_memory', zh: '从原始对话文本或笔记自动捕获记忆。若配置了 LLM(llm_url)，文本会被智能抽取为结构化记忆项（content+tags+importance）后入库；否则启发式回退为按句切分并去重入库。所有捕获项打 auto-captured 标签。支持 text 或 messages 数组两种入参（messages 为结构化对话，会被拼接为带角色标签文本后送入抽取管线）。', description: 'Auto-capture memories from raw conversation text or notes. If an LLM (llm_url) is configured, the text is intelligently extracted into structured memory items (content+tags+importance) before storage; otherwise a heuristic fallback splits into sentences and stores novel chunks with dedup applied. All captured items are tagged auto-captured.',
    inputSchema: { type: 'object', properties: {
      text: { type: 'string', zh: '要捕获的原始对话文本、笔记或转录内容；也可省略 text 改传 messages。', description: 'Raw conversation text, notes, or transcript to capture from.' },
      messages: { type: 'array', items: { type: 'object', properties: { role: { type: 'string', description: '消息角色，如 user / assistant / system。' }, content: { type: 'string', description: '该条消息的文本。' } }, required: ['content'] }, description: 'v1.22.3 结构化对话数组；与 text 二选一或并存（messages 优先）。每项 {role, content}，会被拼接为带角色标签的转写文本后再送入抽取管线。' },
      user: { type: 'string' }, project: { type: 'string' }, session: { type: 'string' },
