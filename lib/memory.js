...
async function purgeMemories(scope) {
  ...
  if (Q()) {
    ...
    const fa = { must: scopeConds.concat(ea.must).concat([{ key: 'pinned', match: { value: false } }]) };
    const fb = { must: scopeConds.concat(eb.must).concat([{ key: 'pinned', match: { value: false } }]) };
    try {
      const ids = (await qdrant.scrollAll(fa)).map(p => p.id).concat((await qdrant.scrollAll(fb)).map(p => p.id));
      const c = ids.length;
