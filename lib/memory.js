...
async function doAdd(a) {
  ...
  if (config.CONFIG.embedding_url) { try { doc.embedding = await embed.embed(a.content); } catch (e) { console.warn('[ai-memory] 嵌入失败（' + (e && e.message) + '），将尝试零向量占位写入 Qdrant'); } }
  ...
  if (Q()) {
    ...
    if (!vec) { try { const sz = await qdrant.vectorSize(); if (sz) vec = new Array(sz).fill(0); } catch (e) {} }
    if (vec) {
  ...
  if (config.CONFIG.lifecycle_policy === 'expire' && config.CONFIG.expiry_days > 0) {
    const now = Date.now();
    if (now - _lastCleanupAt >= CLEANUP_THROTTLE_MS) { _lastCleanupAt = now; try { await cleanupExpired(); } catch (e) { backend.errStats.cleanup++; } }
  }
...
async function cleanupExpired() {
  ...
  const now = Date.now();
  if (now - _lastCleanup < 30000) return 0; // 轻量节流：避免每次写入都跑全集合清理
  _lastCleanup = now;
