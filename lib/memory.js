...
async function searchProject(project, a, vec) {
  ...
  if (ftsMap === null) {
    ...
    return finalize(intelligence.applyRecency(intelligence.rerankWithContext(rows, a.query || '')), a);
  }
...
async function doSearch(a) {
  ...
  let allRows = [], usages = [];
  if (!Q()) {
    let rows = sqliteSearch(a); usages.push(rows.usage); allRows = finish(rows.rows);
  } else {
    const mode = a.mode || 'keyword';
...
async function doList(a) {
  ...
  const out = intelligence.applyRecency(intelligence.rerankWithContext(rows, '')).slice(0, a.limit || 20);
  return { rows: out, usage: { tokens } };
}
