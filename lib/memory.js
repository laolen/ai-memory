...
function _getSearchCache(key) {
  ...
  if (!e) return null;
  if (Date.now() - e.ts > (config.CONFIG.search_cache_ttl_ms || 60000)) { _searchCache.delete(key); return null; }
  return e.data;
}
...
function _searchCacheStats() { return { size: _searchCache.size, enabled: config.CONFIG.search_cache_enabled !== false }; }
