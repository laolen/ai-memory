...
async function batchAdd(items) {
  ...
  const results = [];
  // v1.20.0 (#4a): 批量嵌入——先一次性嵌入所有非 working 记忆的 content，再逐条 doAdd（嵌入结果通过 _embedding 传入）
  const embedableIdx = [];
  const embedableTexts = [];
  for (let i = 0; i < items.length; i++) {
    const it = items[i];
    if (it && it.content && it.tier !== 'working' && !it._embedding) {
      embedableIdx.push(i);
      embedableTexts.push(it.content);
    }
  }
  if (embedableTexts.length > 1 && config.CONFIG.embedding_url) {
    try {
      const vecs = await embed.embedBatch(embedableTexts);
      for (let j = 0; j < embedableIdx.length && j < vecs.length; j++) {
        items[embedableIdx[j]]._embedding = vecs[j];
      }
    } catch (e) { /* 批量嵌入失败，回退逐条嵌入 */ }
  }
