...
// v1.22.0: 运行时可达性缓存——health() 探测后更新；用于「配置了 Qdrant 但运行时不可达」时自动降级 SQLite。
let _reachable = null;
function isReachable() { return _reachable; }
...
async function count(filter) {
  // v1.22.2: exact:true 才真正按 filter 计数；exact:false 会忽略 filter 返回集合总数（导致 total_count 失真）
  const j = await req('POST', `/collections/${coll()}/points/count`, { filter: filter || undefined, exact: true });
...
async function health() {
  try { const j = await req('GET', '/collections/' + coll()); const ok = !!(j && j.result); _reachable = ok; return ok; } catch (e) { _reachable = false; return false; }
}
...
module.exports = {
  useQdrant, isReachable, upsert, get, query, scroll, scrollAll, count, vectorSize, deleteIds, deleteByFilter, setPayload, incrAccess, health, ensureIndexes,
};
