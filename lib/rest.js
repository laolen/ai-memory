...
function createHttpServer() {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    // v1.13.0: 认证中间件——api_keys 非空时检查 Bearer token。admin/docs/health/message/sse 豁免
    const skipAuth = ['/admin', '/api/docs', '/api/health', '/api/diagnose', '/message', '/sse'];
    if (Array.isArray(config.CONFIG.api_keys) && config.CONFIG.api_keys.length && !skipAuth.some(p => url.pathname.startsWith(p))) {
