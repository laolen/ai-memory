...
function createApp() {
  ...
  // 限流：暴露公网后防止暴力调用（全局 100 请求/分钟，破坏性接口更严格）
  ...
  app.register(async function protectedRoutes(instance) {
    ...
    instance.addHook('preHandler', async (req, reply) => {
      ...
      for (const k of keys) {
        try {
          if (k && tok && k.length === tok.length && cryptoMod.timingSafeEqual(Buffer.from(k), Buffer.from(tok))) {
            // 身份通过：写操作需同源校验（防 CSRF 跨站请求）
    ...
    instance.get('/api/tags/cluster', async (req, reply) => {
      const mems = await insight.loadAll();
