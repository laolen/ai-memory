...
function createApp() {
  ...
  app.register(async function protectedRoutes(instance) {
    ...
    instance.post('/api/config', async (req, reply) => {
      const b = req.body;
      const newCfg = {};
    ...
    // ==== 记忆 CRUD ====
    instance.post('/api/memories', async (req, reply) => {
      const b = req.body;
    ...
    instance.put('/api/memories/:id', async (req, reply) => {
      const r = await memory.doUpdate(req.params.id, req.body);
      return r;
    });
    instance.delete('/api/memories/:id', async (req, reply) => {
      await memory.doDelete(req.params.id);
      return { ok: true };
    });
    instance.post('/api/memories/pin', async (req, reply) => {
      const r = await memory.doUpdate(req.body.id, { pinned: true });
      return r;
    });
    instance.post('/api/memories/unpin', async (req, reply) => {
      const r = await memory.doUpdate(req.body.id, { pinned: false });
      return r;
    });
    // 清理过期记忆（按 days/scope）
    instance.delete('/api/memories/cleanup', async (req, reply) => {
    ...
    // 按过滤器批量删除
    instance.delete('/api/memories/filter', async (req, reply) => {
      const b = req.body || {};
    ...
    // ==== 批量操作 ====
    instance.post('/api/capture', async (req, reply) => {
      const b = req.body;
      const r = await capture.captureText(b.text, b);
      return r;
    });
    instance.post('/api/correct/:id', async (req, reply) => {
      const b = req.body;
    ...
    instance.post('/api/project-links', async (req, reply) => {
      const b = req.body;
    ...
    instance.delete('/api/project-links', async (req, reply) => {
      const b = req.body;
      if (!b.from || !b.to) { reply.code(400).send({ error: 'from and to required' }); return; }
    ...
    // ==== 诊断 ====
    instance.post('/api/diagnose', async (req, reply) => {
      const b = req.body || {};
    ...
    // ==== 重索引 ====
    instance.post('/api/reindex', async (req, reply) => {
    ...
    // ==== 巩固 ====
    instance.post('/api/consolidate', async (req, reply) => {
      const b = req.body;
    ...
    instance.post('/api/import', async (req, reply) => {
      const r = await backend.importMemories(req.body.items || []);
      return r;
    });
    instance.post('/api/reset', async (req, reply) => {
      const b = req.body;
      const r = backend.resetMemories(b.confirm);
      return r;
    });
    instance.post('/api/backup', async (req, reply) => {
      const b = req.body || {};
    ...
    instance.post('/api/memories/working', async (req, reply) => { return await memory.addWorking(req.body); });
    instance.delete('/api/memories/working', async (req, reply) => {
      const { id } = req.body || {};
      if (!id) { reply.code(400).send({ error: 'id required' }); return; }
      memory.deleteWorking(id);
      return { ok: true };
    });
    instance.post('/api/memories/working/promote', async (req, reply) => { return await memory.promoteWorking(req.body.id, req.body); });
    instance.get('/api/memories/working/search', async (req, reply) => { return memory.searchWorking(req.query); });

    // ==== KV ====
    instance.post('/api/kv', async (req, reply) => {
    ...
    instance.delete('/api/kv/:key', async (req, reply) => {
      const org = req.query.org || '';
