...
const DOCS = {
  ...
  rest_api: [
    ...
    { method: 'POST', path: '/api/memories/batch', desc: 'v1.11.0 批量新增：body:{items:[{content,user,project,session,tags,tier,org,mem_category,...}]}。逐条走 doAdd（working 记忆也支持）。返回 {added,results}。' },
    { method: 'DELETE', path: '/api/memories/filter', desc: 'v1.11.0 按通用过滤器批量删除：body:{filters:{all:[...]|any:[...]|not:cond|leaf:{key,op,value}}, user?, project?, session?}。op∈eq,ne,gt,gte,lt,lte,contains,in,exists,between。返回 {deleted,ids}。' },
    { method: 'POST', path: '/api/memories/:id/reextract', desc: 'v1.11.0 重新抽取：对某条记忆用新文本重新抽取事实并更新。body:{text, extract_instructions?}。' },
    { method: 'GET', path: '/api/memories?filters=&include_working=&tier=&org=&extract_version=', desc: 'v1.11.0 检索增强：filters 为嵌套过滤 DSL（JSON）；include_working=1 并入短时工作记忆；tier/org/extract_version 为新字段过滤。响应新增 usage.tokens（token 估算）。' },
    { method: 'GET/POST/DELETE', path: '/api/kv?key=&org=', desc: 'v1.11.0 KV 精确匹配通道：flag/配置/短字段的确定性精确查（与语义检索解耦）。GET 列出/取值，POST/PUT 写入 {key,value,org?}，DELETE 删除 {key,org?}。' },
    { method: 'POST', path: '/api/working', desc: 'v1.11.0 写入短时工作记忆：body:{content,user,project,session,org?,tags?}。独立于长期库，不污染 Qdrant/FTS/KG/审计，默认按 working_ttl_hours 过期。' },
    { method: 'GET', path: '/api/working?project=&session=&org=&q=', desc: 'v1.11.0 列出/搜索短时工作记忆；带 q 按子串搜索。' },
    { method: 'DELETE', path: '/api/working/:id', desc: 'v1.11.0 删除一条短时工作记忆。' },
    { method: 'POST', path: '/api/working/:id/promote', desc: 'v1.11.0 把短时工作记忆提升为长期记忆（promote 到主存储，原 working 删除）。' },
    { method: 'GET/PUT/DELETE', path: '/api/projects/:project/config', desc: 'v1.12.0 项目级持久配置：custom_categories(自定义类别体系)/extract_instructions(持久抽取指令)/criteria(检索加权准则)/webhook_urls(项目级事件推送)。PUT 合并更新，GET /api/projects/config 列出全部。' },
    { method: 'GET', path: '/api/memories?actor_id=&agent_id=&run_id=&criteria=', desc: 'v1.12.0 多角色归属过滤（actor/agent/run）+ criteria 加权检索（JSON 数组 [{text,weight}]，按准则语义相似度加权融合重排）。POST /api/memories 与 /api/capture 同样接受 actor_id/agent_id/run_id。' },
    { method: 'GET', path: '/api/webhooks/recent', desc: 'v1.12.0 webhook 最近投递记录（环形缓冲 50 条，排障用）。事件：memory.added/updated/deleted/promoted/consolidated；目标 = 全局 webhook_urls + 项目级 webhook_urls，失败自动重试 1 次。' },
    ...
    { method: 'POST', path: '/api/memories/pin', desc: 'v1.13.0 记忆固定：body:{id}。pinned 记忆免除过期/衰减/清理。' },
    { method: 'POST', path: '/api/memories/unpin', desc: 'v1.13.0 记忆解固。body:{id}。' },
    { method: 'GET', path: '/api/export?project=&user=&limit=', desc: 'v1.13.0 导出记忆为 JSON 数组（备份/迁移）。' },
    { method: 'POST', path: '/api/import', desc: 'v1.13.0 导入记忆 JSON。body:{items:[{...}]}。' },
    { method: 'POST', path: '/api/reset', desc: 'v1.13.0 重置全部记忆（清空+KG+FTS+working+KV）。body:{confirm:true}。' },
    { method: 'POST', path: '/api/backup', desc: 'v1.13.0 备份记忆到文件。body:{project?,user?}。返回 {file,count}。' },
    { method: 'GET', path: '/api/stats?project=', desc: 'v1.13.0 记忆统计面板数据：总量/固定/过期数/分类分布。' },
  ],
  config_fields: [
    { key: 'qdrant_url', desc: 'Qdrant 节点地址，如 http://192.168.110.248:6333。主存储（向量+结构化 payload）。' },
    ...
    { key: 'extract_version', desc: '抽取模型版本（v1/v2），默认 v2。v1.11.0 新增：v2 在抽取时额外产出 Mem0 式高层语义类别 mem_category（fact/preference/opinion/event/procedure/skill），更贴近个性化。' },
    { key: 'working_ttl_hours', desc: '短时工作记忆（tier=working）自动过期小时数（0=不过期），默认 24。v1.11.0 新增。' },
  ...
  architecture: 'ai-memory.service(Node, :8765) ── 直连 ──> Qdrant(:6333) 作为向量主存储（qdrant_url 留空则降级本地 SQLite 文件库 memories.db）。向量由 embedding 服务（OpenAI /v1/embeddings 兼容，如 llama-embed/Ollama）生成。配置持久化于 config.json。代码以 lib/ 分层模块组织（config/util/embed/backend/intelligence/projects/graph/facts/memory/capture/correction/quality/diagnostics/mcp/rest）。',
  notes: [
    '混合检索使用「应用层 RRF」融合关键词(BM25)与语义(kNN)结果，而非依赖单一后端。',
    '向量在 CPU 上生成，单次嵌入约数百毫秒；修改记忆内容会异步重算向量（失败静默跳过）。',
    '写入后通常可立即检索（SQLite/Qdrant 近实时），偶发毫秒级延迟属正常现象。',
    ...
    'v1.11.0 补齐与 Mem0 的差距（8 项全做）：① 结构化 categories + 版本化抽取模型——v2 抽取管线额外产出 Mem0 式高层语义类别 mem_category(fact/preference/opinion/event/procedure/skill)，extract_version 可切 v1/v2；② 嵌套过滤表达式——search/list 支持通用 metadata 嵌套 AND/OR DSL（all/any/not + 叶子 op），客户端谓词统一覆盖 Qdrant/SQLite 双路径（避开 Qdrant should 非标准结构），响应回报 usage.tokens；③ per-call 抽取引导——add_memory/capture 支持 extract_instructions 控制「记什么/不记什么」；④ 批量/运维——batch_add、按过滤器批量删除(delete_by_filter)、重新抽取(update text)；⑤ KV 精确匹配通道——独立 kv_store 表 + /api/kv 端点（flag/配置短字段确定性精确查，与语义检索解耦）；⑥ 记忆分层——tier(working 短时缓冲/long 长期) + org 作用域，working 记忆独立缓冲不污染长期库，可 promote；⑦ 评测基准——eval/bench_v111.js 提供 category 准确率/dedup/过滤/KV/working 的指标尺子；⑧ 其余 v1.10.0 已追平项保持。',
    'v1.12.0 再补齐 Mem0 差距（4 项全做）：① 项目级持久配置——project_config 表持久化 custom_categories(自定义类别体系，抽取时替换默认 6 类)/extract_instructions(项目级持久抽取指令，capture 自动注入)/criteria/webhook_urls，REST /api/projects/:project/config 管理；② 多角色归属——actor_id/agent_id/run_id 三元归属贯穿 add/capture/搜索过滤（Qdrant payload + SQLite 列双路径）；③ criteria 加权检索——按 [{text,weight}] 准则对候选做语义相似度加权融合重排（进程内嵌入缓存），search 传 criteria 或项目配置注入；④ webhooks 事件推送——零依赖 http/https 火忘式 POST（1 次重试），事件 memory.added/updated/deleted/promoted/consolidated，全局 webhook_urls + 项目级合并去重，/api/webhooks/recent 可查最近投递。',
    'v1.13.0 补齐剩余差距（10 项全做）：① MMR 多样性重排——mmr_enabled+mmr_lambda(λ) 控制语义 vs 多样性平衡，search/list 末次排序前插入；② 可插拔 reranker——reranker_url 指向 cross-encoder 端点，search/list 走 reranker 精排（静默退化）；③ 记忆固定——pinned 字段免除过期清理（purge/cleanup 跳过 pinned 记忆）；④ 自动压缩——auto_compress 开启时 capture 后自动触发 consolidate（非阻塞，作用于该项目）；⑤ 认证——api_keys 数组配置 Bearer token，admin/docs/health 豁免；⑥ 批量导出（GET /api/export JSON）+ 导入（POST /api/import）；⑦ 重置（POST /api/reset?confirm=true）；⑧ 备份（POST /api/backup 写出文件）；⑨ 游标分页——search/list 响应含 next_cursor；⑩ 统计面板（GET /api/stats 返回总量/固定/过期/分类分布）。零新依赖。',
  ],
};
...
function createHttpServer() {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    // v1.14.0: 认证中间件——api_keys 非空时检查 Bearer token（Authorization header 或 ?key= 查询参数）
    // 免鉴权路径：admin（静态 HTML，JS 内部处理登录）和 health（Docker HEALTHCHECK）
    const skipAuth = ['/api/health', '/admin'];
    if (Array.isArray(config.CONFIG.api_keys) && config.CONFIG.api_keys.length && !skipAuth.some(p => url.pathname === p)) {
      const ah = req.headers['authorization'] || '';
      const tok = ah.startsWith('Bearer ') ? ah.slice(7) : (url.searchParams.get('key') || '');
      // 时序安全比较，避免通过响应时间侧信道爆破 token
      const cryptoMod = require('crypto');
      let authed = false;
      for (const k of config.CONFIG.api_keys) {
        try {
          if (k && tok && k.length === tok.length && cryptoMod.timingSafeEqual(Buffer.from(k), Buffer.from(tok))) { authed = true; break; }
        } catch (e) { errC.other++; }
      }
      if (!authed) {
        res.writeHead(401, { 'WWW-Authenticate': 'Bearer' }); res.end('{"error":"unauthorized"}'); return;
      }
    }
    try {
      // --- MCP SSE ---
      ...
      if (url.pathname === '/api/diagnose' && req.method === 'POST') {
        ...
        out.store = qdrant.useQdrant() ? 'qdrant' : 'sqlite';
        if (qdrant.useQdrant()) {
          try {
      ...
      // --- REST: health（带 1 秒缓存，避免 admin 界面每 8 秒轮询打 Qdrant）---
      if (url.pathname === '/api/health' && req.method === 'GET') {
        const now = Date.now();
        if (_healthCache && _healthCache.ts && now - _healthCache.ts < 1000) {
          return sendJson(res, 200, _healthCache.body);
        }
        let qOk = false;
      ...
      if (url.pathname === '/api/config' && req.method === 'GET') {
        const masked = { ...config.CONFIG,
          ...
          reranker_api_key: config.CONFIG.reranker_api_key ? '******' : '',
          api_keys: (config.CONFIG.api_keys && config.CONFIG.api_keys.length) ? '******' : [] };
      ...
      if (url.pathname === '/api/config' && req.method === 'POST') {
        ...
        // ---- v1.11–1.13 新增配置项（此前 POST 未处理，导致管理界面改了不落盘）----
        if (b.extract_version !== undefined) newCfg.extract_version = b.extract_version || 'v2';
        if (b.working_ttl_hours !== undefined) newCfg.working_ttl_hours = Number(b.working_ttl_hours) || 24;
        if (b.webhook_enabled !== undefined) newCfg.webhook_enabled = b.webhook_enabled;
        if (b.webhook_urls !== undefined && Array.isArray(b.webhook_urls)) newCfg.webhook_urls = b.webhook_urls;
        if (b.webhook_timeout_ms !== undefined) newCfg.webhook_timeout_ms = Math.max(500, Number(b.webhook_timeout_ms) || 5000);
        if (b.mmr_enabled !== undefined) newCfg.mmr_enabled = b.mmr_enabled;
        if (b.mmr_lambda !== undefined) newCfg.mmr_lambda = Number(b.mmr_lambda) || 0.3;
        if (b.reranker_url !== undefined) newCfg.reranker_url = b.reranker_url;
        if (b.reranker_model !== undefined) newCfg.reranker_model = b.reranker_model || '';
        if (b.reranker_api_key !== undefined && b.reranker_api_key !== '******') newCfg.reranker_api_key = b.reranker_api_key;
        if (b.auto_compress !== undefined) newCfg.auto_compress = b.auto_compress;
        if (b.backup_path !== undefined && b.backup_path) newCfg.backup_path = b.backup_path;
        if (b.api_keys !== undefined) { if (b.api_keys !== '******' && Array.isArray(b.api_keys)) newCfg.api_keys = b.api_keys; }
        config.saveConfig(newCfg);
        config.reload(); // 立即更新进程内配置：非 systemd 部署（如 Windows 直接 node）也能即时生效，无需重启
        sendJson(res, 200, { ok: true, restarting: false, config: newCfg });
        const { exec } = require('child_process');
        // 兼容 systemd 部署：best-effort 重启（非 systemd 环境会静默失败，上面已 reload 内存配置）
        setTimeout(() => { try { exec('systemctl restart ai-memory'); } catch (e) { errC.other++; } }, 400);
        return;
      }
      // --- REST: auto-capture ---
      if (url.pathname === '/api/capture' && req.method === 'POST') {
        ...
        const r = await capture.captureText(b.text || '', { user: b.user, project: b.project, session: b.session, tags: b.tags, source: b.source, memory_type: b.memory_type,
          extract_instructions: b.extract_instructions, org: b.org, tier: b.tier, extract_version: b.extract_version,
          actor_id: b.actor_id, agent_id: b.agent_id, run_id: b.run_id, custom_categories: b.custom_categories }); // v1.12.0 (gap①②)
      ...
      if (url.pathname === '/api/reindex' && req.method === 'POST') {
        ...
        try { kg = await backend.kgReindexAll(); } catch (e) { errC.other++; }
        try { ch = await backend.backfillContentHash(); } catch (e) { ch = -1; }
        return sendJson(res, 200, { ok: true, fts_indexed: fts, kg, content_hash_backfilled: ch });
      }
      ...
      // --- REST: 项目级持久配置（v1.12.0 gap①：custom_categories/extract_instructions/criteria/webhook_urls）---
      if (url.pathname === '/api/projects/config' && req.method === 'GET') {
        return sendJson(res, 200, { ok: true, items: backend.projectConfigList() });
      }
      const pcM = url.pathname.match(/^\/api\/projects\/([^/]+)\/config$/);
      if (pcM) {
        const proj = decodeURIComponent(pcM[1]);
        if (req.method === 'GET') {
          const c = backend.projectConfigGet(proj);
          if (!c) return sendJson(res, 404, { ok: false, error: 'no config for project: ' + proj });
          return sendJson(res, 200, { ok: true, project: proj, config: c });
        }
        if (req.method === 'PUT' || req.method === 'POST') {
          const b = await readBody(req);
          const r = backend.projectConfigSet(proj, {
            custom_categories: b.custom_categories, extract_instructions: b.extract_instructions,
            criteria: b.criteria, webhook_urls: b.webhook_urls
          });
          return sendJson(res, 200, { ok: true, project: proj, config: r });
        }
        if (req.method === 'DELETE') {
          backend.projectConfigDelete(proj);
          return sendJson(res, 200, { ok: true, project: proj, deleted: true });
        }
      }
      // --- REST: webhook 最近投递记录（v1.12.0 gap④，排障用）---
      if (url.pathname === '/api/webhooks/recent' && req.method === 'GET') {
        return sendJson(res, 200, { ok: true, enabled: !!config.CONFIG.webhook_enabled, deliveries: webhook.recentDeliveries() });
      }
      ...
      if (url.pathname === '/api/memories' && req.method === 'POST') {
        ...
        const r = await memory.doAdd({
          ...
          type: b.type, category: b.category, mem_category: b.mem_category, confidence: b.confidence,
          memory_type: b.memory_type, tier: b.tier, org: b.org, extract_version: b.extract_version,
          actor_id: b.actor_id, agent_id: b.agent_id, run_id: b.run_id, // v1.12.0 (gap②)
          pinned: b.pinned, // v1.13.0
          expires_at: b.expires_at, source: b.source, fact_entities: b.fact_entities, merge: b.merge
        });
        return sendJson(res, 200, { ok: true, ...r });
      }
      // --- REST: 批量新增（v1.11.0 gap⑤）---
      if (url.pathname === '/api/memories/batch' && req.method === 'POST') {
        const b = await readBody(req);
        const r = await memory.batchAdd((b.items || []).map(it => Object.assign({}, it, { org: it.org || b.org || null, tier: it.tier || b.tier || 'long' })));
        return sendJson(res, 200, { ok: true, ...r });
      }
      // --- REST: 重新抽取（v1.11.0 gap⑤：update(memory_id, text)）---
      const reEx = url.pathname.match(/^\/api\/memories\/(.+)\/reextract$/);
      if (reEx && req.method === 'POST') {
        const id = decodeURIComponent(reEx[1]);
        const b = await readBody(req);
        const r = await memory.reextractMemory(id, b.text || '', { extract_instructions: b.extract_instructions, extract_version: b.extract_version });
        return sendJson(res, 200, { ok: true, ...r });
      }
      // --- REST: 按过滤器批量删除（v1.11.0 gap⑤）---
      if (url.pathname === '/api/memories/filter' && req.method === 'DELETE') {
        const b = await readBody(req);
        const scope = { user: b.user || url.searchParams.get('user') || '', project: b.project || url.searchParams.get('project') || '', session: b.session || url.searchParams.get('session') || '' };
        const r = await memory.deleteByFilter(b.filters || null, scope);
        return sendJson(res, 200, { ok: true, ...r });
      }
      // --- REST: KV 精确匹配通道（v1.11.0 gap③）---
      if (url.pathname === '/api/kv') {
        const org = url.searchParams.get('org') || '';
        if (req.method === 'GET') {
          const key = url.searchParams.get('key') || '';
          if (key) return sendJson(res, 200, { ok: true, key, org, value: backend.kvGet(key, org) });
          return sendJson(res, 200, { ok: true, org, items: backend.kvList(org) });
        }
        if (req.method === 'POST' || req.method === 'PUT') {
          const b = await readBody(req);
          const k = b.key || url.searchParams.get('key') || '';
          const r = backend.kvSet(k, b.value !== undefined ? b.value : url.searchParams.get('value'), b.org || org);
          return sendJson(res, 200, r);
        }
        if (req.method === 'DELETE') {
          const b = await readBody(req);
          const k = b.key || url.searchParams.get('key') || '';
          const r = backend.kvDelete(k, b.org || org);
          return sendJson(res, 200, r);
        }
      }
      // --- REST: 短时工作记忆缓冲（v1.11.0 gap①）---
      if (url.pathname === '/api/working' && req.method === 'POST') {
        const b = await readBody(req);
        const r = await memory.addWorking({ content: b.content, user: b.user, project: b.project, session: b.session, org: b.org, tags: b.tags, memory_type: b.memory_type, source: b.source, mem_category: b.mem_category, expires_at: b.expires_at });
        return sendJson(res, 200, { ok: true, ...r });
      }
      if (url.pathname === '/api/working' && req.method === 'GET') {
        const project = url.searchParams.get('project') || '';
        const session = url.searchParams.get('session') || '';
        const user = url.searchParams.get('user') || '';
        const org = url.searchParams.get('org') || '';
        const q = url.searchParams.get('q') || '';
        if (q) return sendJson(res, 200, { count: 0, rows: memory.searchWorking({ project, session, user, org, query: q }) });
        return sendJson(res, 200, { count: 0, rows: memory.listWorking({ project, session, user, org }) });
      }
      const wkDel = url.pathname.match(/^\/api\/working\/(.+)$/);
      if (wkDel && req.method === 'DELETE') {
        const id = decodeURIComponent(wkDel[1]);
        return sendJson(res, 200, { ok: true, ...memory.deleteWorking(id) });
      }
      const wkProm = url.pathname.match(/^\/api\/working\/(.+)\/promote$/);
      if (wkProm && req.method === 'POST') {
        const id = decodeURIComponent(wkProm[1]);
        const b = await readBody(req);
        const r = await memory.promoteWorking(id, { org: b.org });
        return sendJson(res, 200, { ok: true, ...r });
      }
      ...
      if (url.pathname === '/api/memories' && req.method === 'GET') {
        ...
        const incWork = url.searchParams.get('include_working');
        const org = url.searchParams.get('org') || '';
        const tier = url.searchParams.get('tier') || '';
        const ev = url.searchParams.get('extract_version') || '';
        const actorId = url.searchParams.get('actor_id') || '';   // v1.12.0 (gap②)
        const agentId = url.searchParams.get('agent_id') || '';
        const runId = url.searchParams.get('run_id') || '';
        let criteria = null;                                      // v1.12.0 (gap③)
        const cstr = url.searchParams.get('criteria');
        if (cstr) { try { criteria = JSON.parse(cstr); } catch (e) { criteria = null; } }
        let filters = null;
        const fstr = url.searchParams.get('filters');
        if (fstr) { try { filters = JSON.parse(fstr); } catch (e) { filters = null; } }
        const filt = { limit, project, user, session, from, to };
        ...
        if (ev) filt.extract_version = ev;
        if (actorId) filt.actor_id = actorId;
        if (agentId) filt.agent_id = agentId;
        if (runId) filt.run_id = runId;
        if (criteria) filt.criteria = criteria;
        if (incRel !== null) filt.include_related = (incRel === 'true' || incRel === '1');
        if (incWork !== null) filt.include_working = (incWork === 'true' || incWork === '1');
        if (filters) filt.filters = filters;
        let out;
        if (q) out = await memory.doSearch({ query: q, mode, top_k: limit, project, user, session, from, to, ...filt });
        else out = await memory.doList(filt);
        return sendJson(res, 200, { count: out.rows.length, rows: out.rows, next_cursor: out.next_cursor || null, usage: out.usage });
      }
      ...
      if (url.pathname === '/api/memories/pin' && req.method === 'POST') {
        const b = await readBody(req);
        const r = await memory.doUpdate(b.id, { pinned: true });
        return sendJson(res, 200, { ok: true, id: b.id, pinned: true, ...r });
      }
      if (url.pathname === '/api/memories/unpin' && req.method === 'POST') {
        const b = await readBody(req);
        const r = await memory.doUpdate(b.id, { pinned: false });
        return sendJson(res, 200, { ok: true, id: b.id, pinned: false, ...r });
      }
      // --- v1.13.0: 批量导出/导入/重置/备份/统计 ---
      if (url.pathname === '/api/export' && req.method === 'GET') {
        const scope = { user: url.searchParams.get('user') || '', project: url.searchParams.get('project') || '', session: url.searchParams.get('session') || '', limit: parseInt(url.searchParams.get('limit') || '10000', 10) };
        const items = await backend.exportMemories(scope);
        return sendJson(res, 200, { ok: true, count: items.length, items });
      }
      if (url.pathname === '/api/import' && req.method === 'POST') {
        const b = await readBody(req);
        const r = await backend.importMemories(b.items || []);
        return sendJson(res, 200, { ok: true, ...r });
      }
      if (url.pathname === '/api/reset' && req.method === 'POST') {
        const b = await readBody(req);
        try { const r = backend.resetMemories(b.confirm === true || b.confirm === 'true'); return sendJson(res, 200, { ok: true, ...r }); }
      ...
      if (url.pathname === '/api/backup' && req.method === 'POST') {
        ...
        if (!bp) return sendJson(res, 400, { ok: false, message: 'backup_path 无效或不在 ROOT 之下' });
        ...
        return sendJson(res, 200, { ok: true, file: fn, count: items.length });
      }
      if (url.pathname === '/api/stats' && req.method === 'GET') {
        ...
        if (qdrant.useQdrant() && config.CONFIG.embedding_url) {
          ...
          try {
            ...
            const r = d.prepare('SELECT COUNT(*) as c FROM memories').get(); if (r) total = r.c;
            const r2 = d.prepare('SELECT COUNT(*) as c FROM memories WHERE pinned=1').get(); if (r2) pinned = r2.c;
            const r3 = d.prepare('SELECT COUNT(*) as c FROM memories WHERE expires_at IS NOT NULL AND expires_at < ?').get(new Date().toISOString()); if (r3) expired = r3.c;
            const rows = d.prepare('SELECT mem_category, COUNT(*) as cnt FROM memories GROUP BY mem_category ORDER BY cnt DESC').all();
            catDist = rows;
          } catch (e) { errC.other++; }
        }
        return sendJson(res, 200, { ok: true, server_version: config.SERVER_VERSION, memories: { total, pinned, expired, by_category: catDist } });
      }
...
function startServer() {
  ...
  _httpServer.timeout = 30000; // v1.14.0: 30 秒请求超时，防止 LLM 卡死时连接泄漏
  _httpServer.listen(config.PORT, () => {
...
async function shutdown(signal) {
  ...
  try { _httpServer && _httpServer.close(); } catch (e) { errC.other++; }
  try { capture.stopWatcher(); } catch (e) { errC.other++; }
  // 等待未完成的写操作完成
...
module.exports = { startServer, shutdown, createHttpServer, DOCS };
