...
const llmProxy = require('./llm_proxy');   // v1.23.0: LLM 代理旁路（无感自动记忆捕获）
...
// ---- v1.22.0 (P1-3): 全局请求限流（固定窗口 / 按客户端 IP / 内存态）----
// 防御性稳定性措施：挡住客户端/智能体失控洪水冲垮服务（内部 LAN 服务，非安全边界）。
// 与受保护路由的 @fastify/rate-limit(60/min) 是两层独立限制，本层覆盖全部请求（含未鉴权端点）。
// 设计为同步、零额外依赖、不触碰连接对象，规避早期 @fastify/rate-limit 全局化时的 ECONNRESET 回归。
const _rateBuckets = new Map(); // ip -> { count, start }
const _RL_EXEMPT = ['/api/health', '/metrics', '/api/docs', '/docs', '/admin', '/'];
function rateLimitCheck(ip, max, windowMs) {
  if (!max || max <= 0) return null; // 关闭
  const now = Date.now();
  let b = _rateBuckets.get(ip);
  if (!b || now - b.start >= windowMs) { b = { count: 0, start: now }; _rateBuckets.set(ip, b); }
  b.count++;
  if (b.count > max) {
    const left = Math.ceil((b.start + windowMs - now) / 1000);
    return { retryAfter: Math.max(1, left) };
  }
  // 防止内存无限增长：桶数过多时清掉已过期项
  if (_rateBuckets.size > 10000) { for (const [k, v] of _rateBuckets) if (now - v.start >= windowMs) _rateBuckets.delete(k); }
  return null;
}
...
const DOCS = {
  ...
  rest_api: [
    ...
    { method: 'GET', path: '/api/config/public', desc: 'v1.22.0 公开连接契约（无密钥，供多客户端漂移检测）。' },
    ...
    { method: 'GET', path: '/api/backups', desc: 'v1.21.0 列出所有备份。' },
    { method: 'GET', path: '/api/backups/download/:name', desc: 'v1.21.0 下载备份文件。' },
    { method: 'POST', path: '/api/backups/restore', desc: 'v1.21.0 从备份恢复记忆。' },
    { method: 'GET', path: '/api/tags', desc: 'v1.21.0 列出所有标签及使用次数。' },
    { method: 'POST', path: '/api/tags/rename', desc: 'v1.21.0 重命名标签（所有记忆同步更新）。' },
    { method: 'POST', path: '/api/tags/delete', desc: 'v1.21.0 删除标签（从所有记忆移除）。' },
    { method: 'GET', path: '/api/scheduler/history', desc: 'v1.21.0 调度器最近执行记录。' },
    { method: 'GET', path: '/api/search-cache/stats', desc: 'v1.21.0 搜索缓存命中率统计。' },
    ...
    { method: 'POST', path: '/llm/v1/chat/completions', desc: 'v1.23.0 LLM 代理旁路：OpenAI 兼容，转发真实 LLM 并自动捕获整段对话（用户提问+助手回答）。客户端 base_url 指向 /llm/v1 即开启无感记忆。' },
    { method: 'GET', path: '/llm/v1/models', desc: 'v1.23.0 返回代理配置的模型列表（OpenAI 客户端兼容）。' },
  ].sort((a, b) => a.path.localeCompare(b.path)),
  config_fields: [
    { key: 'qdrant_url', desc: 'Qdrant 地址（留空自动降级 SQLite）' },
    { key: 'qdrant_collection', desc: 'Qdrant 集合名（默认 memories）' },
    ...
    { key: 'embedding_api_key', desc: 'Embedding API Key（本地 Ollama 留空，云端填 Bearer Token）' },
    { key: 'embedding_timeout_ms', desc: 'Embedding 请求超时毫秒（默认 30000）' },
    ...
    { key: 'capture_min_chars', desc: '捕获单条最小字符数（默认 20）' },
    { key: 'capture_keywords', desc: '捕获关键词/正则白名单（留空=全部捕获）' },
    { key: 'capture_max_per_call', desc: '单次捕获最多入库条数（默认 20）' },
    { key: 'reconcile_enabled', desc: '启用冲突检测与合并（默认开）' },
    { key: 'auto_filter', desc: '捕获前触发 LLM 判定是否值得记（默认关）' },
    { key: 'fact_types', desc: '事实类型本体 JSON 数组（默认 8 类）' },
    { key: 'fact_confidence_threshold', desc: '事实最低置信度 0-1（默认 0.5）' },
    { key: 'kg_enabled', desc: '启用知识图谱抽取（默认关）' },
    { key: 'kg_model', desc: '图谱抽取模型名（留空=复用 LLM 模型）' },
    { key: 'kg_url', desc: '图谱抽取端点 URL（留空=复用 LLM 端点）' },
    { key: 'kg_api_key', desc: '图谱抽取 API Key（留空=复用 LLM Key）' },
    { key: 'kg_max_entities', desc: '单条记忆最大实体数（默认 30）' },
    { key: 'kg_synonyms', desc: '实体同义词归一表 JSON' },
    { key: 'llm_enabled', desc: '启用 LLM 事实抽取/冲突调和（默认开）' },
    { key: 'llm_url', desc: 'LLM OpenAI 兼容端点' },
    { key: 'llm_model', desc: 'LLM 模型名' },
    { key: 'llm_api_key', desc: 'LLM API Key（本地 Ollama 留空，云端填 Bearer Token）' },
    { key: 'llm_timeout_ms', desc: 'LLM 请求超时毫秒（默认 90000）' },
    { key: 'webhook_enabled', desc: '启用 Webhook 投递（默认关）' },
    { key: 'webhook_urls', desc: 'Webhook 目标 URL 数组' },
    { key: 'webhook_timeout_ms', desc: 'Webhook 请求超时毫秒（默认 5000）' },
    { key: 'webhook_secret', desc: 'v1.14.0 Webhook HMAC-SHA256 签名密钥（留空=不签名）' },
    { key: 'reranker_url', desc: 'Reranker 精排端点 URL（留空=不用 reranker）' },
    { key: 'reranker_model', desc: 'Reranker 模型名' },
    { key: 'reranker_api_key', desc: 'Reranker API Key（留空=无鉴权）' },
    { key: 'mmr_enabled', desc: '启用 MMR 多样性重排（默认关）' },
    { key: 'mmr_lambda', desc: 'MMR 权衡 λ，0=纯语义 1=纯多样（默认 0.3）' },
    { key: 'auto_compress', desc: '启用自动压缩/巩固（默认关）' },
    { key: 'working_ttl_hours', desc: '工作记忆存活时长（小时，默认 24）' },
    { key: 'session_ttl_hours', desc: '会话记忆存活时长（小时，默认 0）' },
    { key: 'entity_link_boost', desc: '实体链接加权系数 0-1（默认 0.15）' },
    { key: 'preserve_on_conflict', desc: '冲突时保留旧记忆（默认关）' },
    { key: 'source_trust_enabled', desc: '启用来源信任加权（默认开）' },
    { key: 'source_trust_weights', desc: '来源信任权重 JSON（默认 {"human":1.0,"agent":0.85,"tool":0.7,"system":0.6}）' },
    ...
    { key: 'extract_version', desc: '事实抽取版本 v2（当前）/ v1（兼容旧流程）' },
    { key: 'scheduler_interval_min', desc: 'Scheduler 扫描间隔分钟（默认 30）' },
    ...
    { key: 'salience_w_imp', desc: 'v1.21.0 Salience 重要性权重（默认 0.5）' },
    { key: 'salience_w_acc', desc: 'v1.21.0 Salience 访问强化权重（默认 0.5）' },
    { key: 'salience_access_k', desc: 'v1.21.0 Salience 访问次数归一化分母（默认 10）' },
    { key: 'salience_score_w', desc: 'v1.21.0 Salience 综合分融合权重（默认 0.7）' },
    { key: 'auto_backup_interval_hours', desc: 'v1.21.0 定时备份间隔（小时，0=关闭，默认 0）' },
    { key: 'rate_limit_max', desc: 'v1.22.0 全局限流：每客户端每窗口允许的最大请求数（0/负=关闭，默认 300）' },
    { key: 'rate_limit_window_ms', desc: 'v1.22.0 全局限流窗口长度毫秒（默认 60000）' },
    { key: 'llm_proxy_enabled', desc: 'v1.23.0 启用 LLM 代理旁路（客户端 base_url 指向 /llm/v1 即无感自动捕获对话记忆，默认关）' },
    { key: 'llm_proxy_url', desc: 'v1.23.0 代理上游 LLM 地址（留空=复用 llm_url；支持以 /v1 结尾，自动补 /chat/completions）' },
    { key: 'llm_proxy_model', desc: 'v1.23.0 代理默认模型（客户端请求带 model 时以其为准）' },
    { key: 'llm_proxy_api_key', desc: 'v1.23.0 代理上游 API Key（留空=复用 llm_api_key）' },
    { key: 'llm_proxy_auto_capture', desc: 'v1.23.0 代理响应后是否自动把本轮对话入库（默认开）' },
    { key: 'llm_proxy_capture_scope', desc: "v1.23.1 送入抽取的范围：'turn'（默认，只取本轮 user+assistant）/ 'full'（整段历史，噪声大不推荐）" },
    { key: 'llm_proxy_capture_project', desc: 'v1.23.0 自动捕获归属的项目（缺省时经 X-Project-Path 头或 query.project 覆盖）' },
    { key: 'llm_proxy_user', desc: 'v1.23.0 自动捕获时记忆的归属来源（默认 assistant）' },
  ],
  search_modes: [
...
function createApp() {
  ...
  // v1.22.0 (P1-3): 全局请求限流 onRequest 钩子（固定窗口 / 按 IP / 内存态）
  // 默认 300/min/客户端；0 或负关闭。运维/健康检查类端点豁免，避免监控探针被自己限流。
  app.addHook('onRequest', async (req, reply) => {
    const cfg = config.CONFIG;
    const max = cfg.rate_limit_max || 0;
    if (!max || max <= 0) return; // 关闭
    const path = (req.url || '').split('?')[0];
    if (_RL_EXEMPT.includes(path)) return;
    const hit = rateLimitCheck(clientIp(req), max, cfg.rate_limit_window_ms || 60000);
    if (hit) {
      reply.header('Retry-After', String(hit.retryAfter));
      reply.code(429).send({ error: 'rate_limited', message: '请求过于频繁，请稍后重试', retry_after: hit.retryAfter });
    }
  });
  ...
  app.get('/api/health', async (req, reply) => {
    ...
    // v1.22.0: 配置了 Qdrant 但不可达时，降级显示为 sqlite（fallback），而非误导性地标 qdrant
    const store = (qdrant.useQdrant() && qOk) ? 'qdrant' : 'sqlite';
    const body = {
      ...
      config: { qdrant_url: config.CONFIG.qdrant_url, qdrant_collection: config.CONFIG.qdrant_collection,
        ...
        llm_proxy_enabled: config.CONFIG.llm_proxy_enabled } };
  ...
  instance.addHook('preHandler', async (req, reply) => {
      ...
      // v1.23.0: LLM 代理旁路是 API-to-API 调用（OpenAI SDK 不发 X-Requested-With），同样豁免 CSRF 头要求
      const isProxy = p.startsWith('/llm/');
      // v1.22.0 (P2-6): 公开连接契约端点豁免鉴权——多客户端漂移检测不应要求密钥
      // （与 /api/health 同属运维探针；health 在钩子注册前登记本就免鉴权，此处显式补齐契约端点）。
      if (p === '/api/config/public') { req.authScope = null; return; }
      ...
      // 身份通过：REST 写操作需同源校验（防 CSRF 跨站请求）；MCP 端点（SDK/非浏览器）与 LLM 代理旁路豁免该 header 要求
      if (!isMcp && !isProxy && req.method !== 'GET' && !req.query.key) {
  ...
  instance.addHook('preHandler', async (req, reply) => {
    ...
    instance.get('/api/config', async (req, reply) => {
      ...
      if (c.llm_proxy_api_key) c.llm_proxy_api_key = '******';
      if (c.kg_api_key) c.kg_api_key = '******';
      return c;
    });
    // v1.22.0 (P2-6): 公开连接契约——供多客户端（WorkBuddy / opencode / 各 MCP 客户端）做互操作性
    // 与配置漂移检测。仅含非密连接信息与「密钥是否存在」标志，绝不回传任何密钥明文/掩码值。
    instance.get('/api/config/public', async (req, reply) => {
      const cc = config.CONFIG;
      const store = (qdrant.useQdrant() && cc.embedding_url) ? 'qdrant' : 'sqlite';
      return {
        server_version: config.SERVER_VERSION,
        store,
        qdrant_url: cc.qdrant_url || null,
        qdrant_collection: cc.qdrant_collection || null,
        embedding_url: cc.embedding_url || null,
        embedding_model: cc.embedding_model || null,
        llm_enabled: !!cc.llm_enabled,
        llm_url: cc.llm_url || null,
        llm_model: cc.llm_model || null,
        kg_enabled: !!cc.kg_enabled,
        kg_url: cc.kg_url || null,
        kg_model: cc.kg_model || null,
        search_cache_enabled: cc.search_cache_enabled !== false,
        search_cache_ttl_ms: cc.search_cache_ttl_ms != null ? cc.search_cache_ttl_ms : 60000,
        api_keys_enabled: !!(cc.api_keys && cc.api_keys.length),
        rate_limit_max: cc.rate_limit_max != null ? cc.rate_limit_max : 300,
        rate_limit_window_ms: cc.rate_limit_window_ms != null ? cc.rate_limit_window_ms : 60000,
        auto_backup_enabled: !!cc.auto_backup_enabled,
        auto_backup_interval_hours: cc.auto_backup_interval_hours != null ? cc.auto_backup_interval_hours : 24,
        audit_enabled: cc.audit_enabled !== false,
        // v1.23.0: LLM 代理旁路连接契约
        llm_proxy_enabled: !!cc.llm_proxy_enabled,
        llm_proxy_url: (cc.llm_proxy_url || cc.llm_url || null),
        llm_proxy_auto_capture: cc.llm_proxy_auto_capture !== false,
        llm_proxy_capture_scope: cc.llm_proxy_capture_scope || 'turn',
      };
    });
    instance.post('/api/config', strictRate, async (req, reply) => { reply.code(200);
      ...
      for (const k of ['qdrant_url', 'qdrant_collection', 'embedding_url', 'embedding_model', 'embedding_api_key', 'embedding_timeout_ms',
        ...
        // v1.21.0: salience 可配置权重
        'salience_w_imp', 'salience_w_acc', 'salience_access_k', 'salience_score_w',
        'auto_backup_interval_hours',
        // v1.22.0 (P1-3): 全局限流
        'rate_limit_max', 'rate_limit_window_ms',
        // v1.23.0: LLM 代理旁路
        'llm_proxy_enabled', 'llm_proxy_url', 'llm_proxy_model', 'llm_proxy_api_key', 'llm_proxy_auto_capture', 'llm_proxy_capture_project', 'llm_proxy_user', 'llm_proxy_capture_scope']) {
    ...
    instance.get('/api/memories', async (req, reply) => {
      ...
      if (!q) {
        ...
        return { count: out.total_count || out.rows.length, rows: out.rows, next_cursor: out.next_cursor || null, usage: out.usage };
      }
      ...
      let out;
      try {
        out = await memory.doSearch({ ...cleaned, query: q, mode, limit, filters });
      } catch (e) {
    ...
    // ==== v1.21.0: 标签管理 ====
    instance.get('/api/tags', async (req, reply) => {
      const all = await backend.exportMemories({});
      const freq = {};
      for (const m of all) {
        if (Array.isArray(m.tags)) for (const t of m.tags) { freq[t] = (freq[t] || 0) + 1; }
      }
      return Object.entries(freq).map(([tag, count]) => ({ tag, count })).sort((a, b) => b.count - a.count);
    });
    instance.post('/api/tags/rename', async (req, reply) => { reply.code(200);
      const { from, to } = req.body || {};
      if (!from || !to) { reply.code(400).send({ ok: false, error: 'from and to required' }); return; }
      const all = await backend.exportMemories({});
      let renamed = 0;
      for (const m of all) {
        if (Array.isArray(m.tags) && m.tags.includes(from)) {
          m.tags = m.tags.map(t => t === from ? to : t);
          await backend.sqliteUpdate(m.id, { tags: m.tags });
          renamed++;
        }
      }
      return { ok: true, renamed };
    });
    instance.post('/api/tags/delete', async (req, reply) => { reply.code(200);
      const { tag } = req.body || {};
      if (!tag) { reply.code(400).send({ ok: false, error: 'tag required' }); return; }
      const all = await backend.exportMemories({});
      let removed = 0;
      for (const m of all) {
        if (Array.isArray(m.tags) && m.tags.includes(tag)) {
    ...
    // ==== v1.21.0: 运维 API ====
    instance.get('/api/scheduler/history', async (req, reply) => { return scheduler.getHistory(); });
    instance.get('/api/search-cache/stats', async (req, reply) => { return memory.getSearchCacheStats ? memory.getSearchCacheStats() : {}; });
    ...
    instance.post('/api/backup', async (req, reply) => { reply.code(200);
      ...
      const st = require('fs').statSync(fn);
      backend.backupRecordAdd(require('path').basename(fn), st.size);
      return { ok: true, file: fn, count: items.length, size: st.size };
    });
    // v1.22.0: 操作审计——全局变更账本查询（支持 op/user/project/trigger 过滤 + 分页）
    instance.get('/api/audit', async (req, reply) => {
      const q = req.query || {};
      return backend.getChangelogAll({
        op: q.op || undefined,
    ...
    // v1.21.0: 备份管理——列表/下载/恢复
    instance.get('/api/backups', async (req, reply) => {
      const bp = util.safePath(config.CONFIG.backup_path || config.ROOT + '/backups');
      const rows = backend.backupRecordList();
      const list = rows.map(r => {
        let exists = false;
        try { exists = require('fs').existsSync(bp + '/' + r.name); } catch (e) {}
        return { name: r.name, size: r.size, created_at: r.created_at, exists };
      });
      return list;
    });
    instance.get('/api/backups/download/:name', async (req, reply) => {
      const name = req.params.name;
      const bp = util.safePath(config.CONFIG.backup_path || config.ROOT + '/backups');
      const fn = bp + '/' + name;
      if (!require('fs').existsSync(fn)) { reply.code(404).send({ error: 'not found' }); return; }
      reply.header('Content-Disposition', 'attachment; filename="' + name + '"');
      reply.type('application/json');
      return require('fs').createReadStream(fn);
    });
    instance.post('/api/backups/restore', async (req, reply) => { reply.code(200);
      const name = req.body && req.body.name;
      if (!name) { reply.code(400).send({ ok: false, error: 'name required' }); return; }
      const bp = util.safePath(config.CONFIG.backup_path || config.ROOT + '/backups');
      const fn = bp + '/' + name;
      if (!require('fs').existsSync(fn)) { reply.code(404).send({ ok: false, error: 'backup not found' }); return; }
      try {
        const data = JSON.parse(require('fs').readFileSync(fn, 'utf8'));
        const r = await backend.importMemories(data.items || []);
        return { ok: true, imported: r.imported, errors: r.errors };
      } catch (e) { return { ok: false, error: e.message }; }
    });
    instance.get('/api/stats', async (req, reply) => {
      const project = backend.normalizeProject(req.query.project || ''); // v1.22.2: 查询侧归一化
    ...
    // ==== v1.23.0: LLM 代理旁路（无感自动记忆捕获）====
    // 客户端把 OpenAI SDK 的 base_url 指向 /llm/v1，用户提问+助手回答整段自动入库。
    instance.post('/llm/v1/chat/completions', async (req, reply) => {
      return llmProxy.handleChatCompletions(req, reply);
    });
    instance.get('/llm/v1/models', async (req, reply) => {
      return llmProxy.handleModels(req, reply);
    });
    instance.get('/llm/v1/chat/completions', async (req, reply) => {
      // OpenAI 客户端偶尔用 GET 探测；返回 405 明确告知仅 POST
      reply.code(405).send({ error: 'method_not_allowed', message: 'use POST /llm/v1/chat/completions' });
    });
...
function startServer() {
  ...
  app.listen({ port: config.PORT, host: '0.0.0.0' }).then(async () => {
    ...
    // v1.22.1 (#137): 冷启动预热 embedding 模型（fire-and-forget，不阻塞启动）
    embed.warmupEmbedding().then(warmed => {
      if (warmed) console.log('[ai-memory] Embedding 模型预热完成');
      else console.log('[ai-memory] Embedding 模型预热未完成（服务继续，后续调用将按需加载）');
    }).catch(() => {});
    ...
    // v1.22.0: 周期探测 Qdrant 可达性，驱动「配置但不可达→SQLite」运行时降级（.unref 不阻止进程退出）
    if (qdrant.useQdrant()) setInterval(() => { qdrant.health().catch(() => {}); }, 30000).unref();
  }).catch(err => {
...
function renderPrometheus() {
  ...
  // v1.23.0: LLM 代理旁路指标
  const mp = llmProxy.stats;
  counter('ai_memory_proxy_total', 'Total LLM proxy requests handled', mp.proxy_total);
  counter('ai_memory_proxy_stream_total', 'Total LLM proxy streaming requests', mp.proxy_stream_total);
  counter('ai_memory_proxy_errors', 'Total LLM proxy upstream or self failures', mp.proxy_errors);
  counter('ai_memory_proxy_capture_total', 'Total conversations auto-captured via proxy', mp.capture_total);
  counter('ai_memory_proxy_capture_ok', 'Auto-captured proxy conversations that stored >=1 fact', mp.capture_ok);
  // 搜索耗时直方图
