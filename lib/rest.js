  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/diagnose' && req.method === 'POST') {
        ...
        if (!config.client) {
          ...
          try {
            ...
            out.es = { ok: true, connected: !!ping, index: config.CONFIG.es_index, docs };
          } catch (e) { out.es = { ok: false, error: String(e && e.message || e) }; }
        }
        out.store = config.CONFIG.es_url ? 'elasticsearch' : 'sqlite';
      ...
      if (url.pathname === '/api/health' && req.method === 'GET') {
        let esOk = false;
        if (config.client) { try { esOk = await config.client.ping(); } catch (e) {} }
        return sendJson(res, 200, {
          ok: true, version: config.SERVER_VERSION, store: config.CONFIG.es_url ? 'elasticsearch' : 'sqlite', es_connected: esOk,
          config: { es_url: config.CONFIG.es_url, es_index: config.CONFIG.es_index, es_user: config.CONFIG.es_user,
      ...
      if (m) {
        ...
        if (req.method === 'GET') {
          try {
            if (config.client) { const r = await config.client.get({ index: config.CONFIG.es_index, id }); return sendJson(res, 200, { id: r._id, ...r._source }); }
