...
const lifecycle = require('./memory_lifecycle'); // v1.22.1 (#139): 过期记忆自动清理
...
async function scanOnce(opts = {}) {
  ...
  // v1.22.1 (#139): 过期记忆自动清理——按 expiry_days + expires_at 自动剔除过期条目
  if (config.CONFIG.auto_cleanup_enabled !== false) {
    try {
      const cleaned = await lifecycle.cleanupExpired();
      if (cleaned > 0) out.cleaned_expired = cleaned;
    } catch (e) { out.cleanup_error = String(e.message || e); }
  }
  try {
  ...
  // v1.21.0: 定时备份
  try {
    const interval = config.CONFIG.auto_backup_interval_hours || 0;
    if (interval > 0) {
      const rows = backend.backupRecordList();
      const lastBak = rows.length ? new Date(rows[0].created_at).getTime() : 0;
      const elapsedHr = (Date.now() - lastBak) / 3600000;
      if (lastBak === 0 || elapsedHr >= interval) {
        const items = await backend.exportMemories({});
        const bp = config.CONFIG.backup_path || './backups';
        try { require('fs').mkdirSync(bp, { recursive: true }); } catch (e) {}
        const fn = bp + '/auto_backup_' + new Date().toISOString().replace(/[:.]/g, '-') + '.json';
        require('fs').writeFileSync(fn, JSON.stringify({ version: 'auto', exported_at: new Date().toISOString(), count: items.length, items }, null, 2));
        const st = require('fs').statSync(fn);
        backend.backupRecordAdd(require('path').basename(fn), st.size);
        out.auto_backup = { file: require('path').basename(fn), count: items.length, size: st.size };
      }
    }
  } catch (e) { out.auto_backup_error = String(e.message || e); }
  return out;
}
